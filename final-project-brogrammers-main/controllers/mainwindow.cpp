...
int selected_prof;
int selected_student_id_from_courses = -1;
int selected_prof_id_from_courses = -1;
//------------------------------------------------------------------------------------------------------------------------------------------------------------------
//export to csv
//To be called on clicking the export button in student
void stud_export_to_csv(){
    std::ofstream os;
    os.open("studentsData.csv");
    for(auto student : database->students)
        {
            qDebug()<<"here";
            qDebug()<<student->get_id();
            os<< student->get_id() << "," << student->getName() << "," << student->getDepartment() << "," <<student->getPhone() << "," << student->getEmail()<<",";
                    os<<student->getGraduation_year()<<",";
                    for(auto course: student->get_courses())
                        os<<course<<","<<student->get_courses_grades()[course];
                    os<<"\n";
        }
    os.close();
}
...
void prof_export_to_csv(){
    std::ofstream os;
    os.open("professorsData.txt");
    for(auto professor : database->professors)
    {
        qDebug()<<"here";
              qDebug()<<professor->get_id();
              os<< professor->get_id() << "," << professor->getName() << "," << professor->getDepartment() << "," <<professor->getPhone() << "," << professor->getEmail()<<",";
              os<<professor->get_phd()<<",";
              for(auto course: professor->get_courses())
                  os<<course<<",";
              os<<"\n";
    }
...
void courses_export_to_csv(){
    std::ofstream os;
    os.open("adminsData.txt", std::ios::app);
    for(auto course : database->courses)
    {
        qDebug()<<"here";
                qDebug()<<course->get_ID();
                os<< course->get_ID()<<","<<course->get_name() << "," << course->get_academic_year() << "," << course->get_code() << "," <<course->get_day() << "," << course->get_department()<<",";
                os<<course->get_hall()<<","<<course->get_type()<<",";
                os<<"\n";    }
...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow) {
    ...
    database->professors = database->get_professors_mock_data();
    ...
    Admin* main_admin = new Admin("Muhamed Mustafa", "IT", "Teaching Assistant", "admin@gmail.com", "123456", "01013708540","Teaching Assistant at CUFE", 27 ,QDateTime::currentDateTime());
...
void MainWindow::on_btn_login_clicked() {
    ...
    {
        // Custom Dialog
        Dialog* dialog = new Dialog(this, "The email or the password you entered doesn't match our records.");
...
void MainWindow::on_btn_add_student_form_clicked() {
    ...
    name_is_valid = valid.name_validate(student_name, message);
    department_is_valid = valid.department_validate(department, message);
    phone_is_valid = valid.phone_validate(phone_number, message);
    email_is_valid = valid.email_validate(email, message);
...
void MainWindow::on_add_prof_form_clicked()
{
    ...
    name_is_valid = valid.name_validate(professor_name, message);
    department_is_valid = valid.department_validate(department, message);
    phone_is_valid = valid.phone_validate(phone_number, message);
    email_is_valid = valid.email_validate(email, message);
    graduation_unv_is_valid = valid.name_validate(graduation_university, message);
    phd_subject_is_valid = valid.name_validate(phd_subject, message);
...
void MainWindow::on_tbl_professors_cellClicked(int row, int column)
{
    ...
    ui->lbl_prof_main_name->setText(prof_data["id"]);
...
void MainWindow::on_tbl_students_cellClicked(int row, int _)
{
    ...
    ui->stnt_code_lbl->setText(student_data["id"]);
...
void MainWindow::on_btn_add_admin_form_clicked()
{
    ...
    name_is_valid = valid.name_validate(admin_name, message);
    title_is_valid = valid.title_validate(admin_title, message);
    phone_is_valid = valid.phone_validate(admin_phone_number, message);
    email_is_valid = valid.email_validate(admin_email, message);
    bio_is_valid = valid.biography_validate(admin_biography, message);
    password_is_valid = valid.password_validate(admin_password, message);
...
void MainWindow::on_comboBox_currentIndexChanged(int index)
{
    ...
    if(filter_yr == "All")
    {
        filtered_studens = database->students;
    }
    else
    {
        for(auto student : database->students)
...
void MainWindow::on_comboBox_2_currentIndexChanged(int index)
{
    ...
    if(department == "All")
    {
        filtered_students = database->students;
    }
    else
    {
        for(auto student : database->students)
    ...
    int tbl_students_rows_count = filtered_students.size();
    ui->tbl_students->setRowCount(tbl_students_rows_count);
    ui->tbl_students->verticalHeader()->setVisible(false);
    ui->tbl_students->setEditTriggers(QAbstractItemView::NoEditTriggers);

    for (auto row = 0; row < tbl_students_rows_count; row++) {
        auto student_data = get_student_data_as_qstrings(filtered_students.at(row));
    ...
    qDebug()<<"reached";
    qDebug()<<QString::fromStdString(std::to_string(filtered_students.size()));
...
void MainWindow::on_btn_add_course_clicked()
{
    ...
    ui->stnt_code_lbl_3->setText("Course - ID");
    ui->ln_edt_stnt_nme_5->setText("");
    ui->ln_edt_stnt_nme_7->setText("");
    ui->ln_edt_stnt_dprtmnt_5->setText("");
    ui->tbl_students_4->setRowCount(0);
    ui->tbl_professors_5->setRowCount(0);
...
void MainWindow::on_pushButton_15_clicked()
{
    ...
    name_is_valid = valid.name_validate(course_name, message);
    department_is_valid = valid.department_validate(course_department, message);
    type_is_valid = valid.type_validate(course_type, message);
    hall_is_valid = valid.hall_validate(course_hall, message);
    ...
    if (name_is_valid && department_is_valid && type_is_valid  && hall_is_valid) {
        ...
        Course *course = new Course(database->courses[row_index_course]->get_ID(),
                                course_name.toStdString(),
                                 ...
                                 course_time.addSecs(5400),
                                    year,
                                     database->courses[row_index_course]->get_num_std() + count_student);
        ...
        ui->tbl_courses->setItem(row_index_course, 4, new QTableWidgetItem(QString::fromStdString(std::to_string(database->courses[row_index_course]->get_num_std()))));
        edit_course = false;
        count_student = 0;
...
void MainWindow::on_comboBox_7_currentIndexChanged(int index)
{
    ...
    for (auto row = 0; row < tbl_courses_rows_count; row++) {
        ...
        ui->tbl_courses->setItem(row, 4, new QTableWidgetItem(QString::fromStdString(std::to_string(filtered_courses.at(row)->get_num_std()))));
    }
...
void MainWindow::on_comboBox_4_currentIndexChanged(int index)
{
    ...
    {
        filtered_professors = database->professors;
    }
    else
    {
        for(auto professor : database->professors)
...
void MainWindow::on_pushButton_18_clicked()
{
    ...
    for(auto student : database->students)
    {
        if(id == student->get_id() && unique_id(existance, id))
        {
            ...
            qDebug()<<"ended";
            found  = 1;
            //not sure how to access the current course
            selected_course->add_student(student);
            selected_course->edit_student_grade(student, -1);
...
void MainWindow::on_tbl_students_4_itemChanged(QTableWidgetItem *item)
{
    ...
    qDebug()<<grade;
    //database->courses[row_index_course]->set_students_to_grades(store_current_student, grade);
...
void MainWindow::on_tbl_courses_cellClicked(int row, int column)
{
    ...
    selected_course = database->courses[row];
...
void MainWindow::on_btn_stud_view_clicked()
{
    ui->stnt_code_lbl_4->setText(ui->stnt_code_lbl->text());
    ui->lbl_stud_name->setText(ui->nm_lbl_stud->text());
    ui->lbl_stud_department->setText(ui->ln_edt_stnt_dprtmnt_2->text());
    ui->lbl_stud_grad->setText(ui->cmb_grd_yr_2->currentText());
    ui->lbl_stud_age->setText(ui->dateEdit_2->text());
    ui->lbl_stud_email->setText(ui->ln_edt_stnt_mail_2->text());
    ui->lbl_stud_phone->setText(ui->ln_edt_stnt_phn_2->text());
    QString ID = ui->stnt_code_lbl->text();
    qDebug()<<ID;
    int rows_courses = 0;
    std::vector<std::string> courses;
    std::map<std::string, float> course_to_grade;
    qDebug()<<"here";
    for(auto student : database->students)
    {
        if(QString::fromStdString(std::to_string(student->get_id())) == ID)
        {
            qDebug()<<"found";
            rows_courses = student->get_courses().size();
            for(auto course : student->get_courses())
            {
                courses.push_back(course);
                course_to_grade = student->get_courses_grades();
            }
        }
    }
    qDebug()<<courses.size();
    QHeaderView* students_table_header = ui->tbl_stud_courses->horizontalHeader();
    students_table_header->setSectionResizeMode(QHeaderView::ResizeMode::Stretch);

    int course_count = rows_courses;
    ui->tbl_stud_courses->setRowCount(course_count);
    ui->tbl_stud_courses->verticalHeader()->setVisible(false);
    ui->tbl_stud_courses->setEditTriggers(QAbstractItemView::NoEditTriggers);
    for (auto row = 0; row < rows_courses; row++) {
        // Change the First col in the ui to be the course name and the second col to be the grade
        ui->tbl_stud_courses->setItem(row, 0, new QTableWidgetItem(QString::fromStdString(courses[row])));
        ui->tbl_stud_courses->setItem(row, 1, new QTableWidgetItem(QString::fromStdString(std::to_string(course_to_grade[courses[row]]))));
    }

    // Navigate to P11_Student_View
    ui->ViewStack->setCurrentIndex(push_navigation(11));
}


void MainWindow::on_btn_prof_view_clicked()
{
    int ID = ui->lbl_prof_main_name->text().toInt();
    ui->stnt_code_lbl_5->setText(ui->lbl_prof_main_name->text());
    ui->lbl_prof_name->setText(ui->ln_edt_prof_nme_2->text());
    ui->lbl_prof_department->setText(ui->ln_edt_prof_dprtmnt_2->text());
    ui->lbl_prof_phd->setText(ui->ln_edt_prof_phd_2->text());
    ui->lbl_prof_age->setText(ui->dateEdit_birth->text());
    ui->lbl_prof_grad_from->setText(ui->ln_edt_prof_grdf_2->text());
    ui->lbl_prof_email->setText(ui->ln_edt_prof_mail_2->text());
    ui->lbl_prof_phone->setText(ui->ln_edt_prof_phn_2->text());
    int rows_courses = 0;
    std::vector<std::string> courses;
    for(auto prof : database->professors)
    {
        if(QString::fromStdString(std::to_string(prof->get_id())) == QString::fromStdString(std::to_string(ID)))
        {
            qDebug()<<"found";
            rows_courses = prof->get_courses().size();
            for(auto course : prof->get_courses())
            {
                courses.push_back(course);
            }
        }
    ...
    QHeaderView* prof_table_header = ui->tbl_stud_courses_2->horizontalHeader();
    prof_table_header->setSectionResizeMode(QHeaderView::ResizeMode::Stretch);

    int course_count = rows_courses;
    ui->tbl_stud_courses_2->setRowCount(course_count);
    ui->tbl_stud_courses_2->verticalHeader()->setVisible(false);
    ui->tbl_stud_courses_2->setEditTriggers(QAbstractItemView::NoEditTriggers);
    for (auto row = 0; row < rows_courses; row++) {
        // Change the First col in the ui to be the course name and the second col to be the grade
        ui->tbl_stud_courses_2->setItem(row, 0, new QTableWidgetItem(QString::fromStdString(courses[row])));
    }

    ui->ViewStack->setCurrentIndex(push_navigation(12));
}
...
{
    courses_export_to_csv();
}


void MainWindow::on_pushButton_14_clicked()
{
    int index_to_delete = row_index_course;
    int course_id = ui->tbl_courses->item(row_index_course, 0)->text().toInt();
    Course* sel_course = NULL;
    ...
    if (sel_course) {
        ...
        // or access el vector w amss7 l course mn gwa kul student

        //database->courses[row_index_course]->stu
        database->courses.erase(database->courses.begin() + index_to_delete);
        //Update the QTable Widget of the courses to display the courses after modifing the database
        int tbl_courses_rows_count = database->courses.size();
        ui->tbl_courses->setRowCount(tbl_courses_rows_count);
        ui->tbl_courses->verticalHeader()->setVisible(false);
        ui->tbl_courses->setEditTriggers(QAbstractItemView::NoEditTriggers);
        ui->ViewStack->setCurrentIndex(push_navigation(9));
        ...
        for (auto row = 0; row < tbl_courses_rows_count; row++) {
            auto course_data = get_course_data_as_qstrings(database->courses.at(row));

            ui->tbl_courses->setItem(row, 0, new QTableWidgetItem(course_data["id"]));
            ui->tbl_courses->setItem(row, 1, new QTableWidgetItem(course_data["name"]));
            ui->tbl_courses->setItem(row, 2, new QTableWidgetItem(course_data["department"]));
            ui->tbl_courses->setItem(row, 3, new QTableWidgetItem(course_data["year"]));
            ui->tbl_courses->setItem(row, 4, new QTableWidgetItem(QString::fromStdString(std::to_string(database->courses[row]->get_num_std()))));
        }

        sel_course->dismiss_course();
    }

}


void MainWindow::on_btn_view_sel_student_clicked()
{
    ui->stnt_code_lbl_4->setText(ui->stnt_code_lbl->text());
    ui->lbl_stud_name->setText(ui->nm_lbl_stud->text());
    ui->lbl_stud_department->setText(ui->ln_edt_stnt_dprtmnt_2->text());
...
void MainWindow::on_btn_delete_sel_student_clicked()
{
    for (auto stud: database->students) {
        if (stud->get_id() == selected_student_id_from_courses) {
            stud->unenroll(selected_course->get_name());
            selected_course->delete_student(stud);
            ui->tbl_students_4->removeRow(ui->tbl_students_4->currentRow());
            count_student--;
        }
    }
...
void MainWindow::on_tbl_students_4_cellClicked(int row, int column)
{
    selected_student = row;
    selected_student_id_from_courses = ui->tbl_students_4->item(row, 0)->text().toInt();
}

void MainWindow::on_tbl_professors_5_cellClicked(int row, int column)
{
    selected_prof = row;
    selected_prof_id_from_courses = ui->tbl_professors_5->item(row, 0)->text().toInt();
}

void MainWindow::on_btn_delete_sel_professor_clicked()
{
    for (auto prof: database->professors) {
        if (prof->get_id() == selected_prof_id_from_courses) {
            prof->unenroll(selected_course->get_name());
            selected_course->delete_professor(prof);
            ui->tbl_professors_5->removeRow(ui->tbl_professors_5->currentRow());
        }
    }
}
...
void MainWindow::on_btn_view_sel_professor_clicked()
{
    int ID = ui->lbl_prof_main_name->text().toInt();
    ui->stnt_code_lbl_5->setText(ui->lbl_prof_main_name->text());
    ui->lbl_prof_name->setText(ui->ln_edt_prof_nme_2->text());
